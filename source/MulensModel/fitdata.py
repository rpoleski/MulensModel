...
class FitData(object):
    ...
    def scale_fluxes(self, source_flux, blend_flux):
        ...
        A ``UserWarning`` is emitted if ``err_flux`` contains negative values, which can happen when ``source_flux``
        or the fitted source flux is negative.
        """
        if self.model.n_sources == 1:
        ...
        if np.any(err_flux < 0.):
            text = ("FitData.scale_fluxes() produced negative flux uncertainties. This happens when source_flux or "
                    "the fitted source flux is negative, which is unphysical and usually indicates a problem with "
                    "the model fit. Downstream code may need to mask these values.")
