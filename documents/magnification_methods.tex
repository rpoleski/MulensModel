...
\item {\tt VBM} -- Bozza (2010) method -- finite source with limb darkening.  Most widely used method nowadays.  Parameters that can be set: {\tt accuracy}, {\tt relative\_accuracy}.  This method replaces \texttt{VBBL} but both names are accepted.
...
Triple lens methods:
\begin{itemize}
\item {\tt VBM\_MULTIPLE} -- As {\tt VBM} but for multiple lenses.  Parameters that can be set: {\tt accuracy}, {\tt relative\_accuracy}, {\tt algorithm}, see Bozza (2025) for details.
\end{itemize}
\end{document}
