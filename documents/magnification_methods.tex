...
\item {\tt point\_source\_point\_lens} -- approximates binary lens as a single lens\footnote{For versions 3.8 and earlier, this calculation used the wrong origin for s>1. This was corrected in v3.9.}.  It is useful when binary lens effects are negligible and binary lens calculations may cause numerical errors, e.g., $q\approx10^{-6}$ and source far from caustics.
\end{itemize}
