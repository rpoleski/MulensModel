...
def generate_model(finite_source=True):
    if finite_source:
        model = mm.Model({'t_0_1': 5000., 'u_0_1': 0.05,
                          't_0_2': 5005., 'u_0_2': 0.003, 'rho_0_2': 0.005,
                          't_E': 30.})
    ...
    model.set_magnification_methods_parameters([5004., 'VBBL', 5006.])
...
def get_mag_string(model):
    ...
    eff_mag_i = model.magnification(ref_time, bandpass='I')
    eff_mag_v = model.magnification(ref_time, bandpass='V')
    ...
    str_1 = '{0:8.4f} {1:8.4f} {2:8.4f} '.format(
        eff_mag, eff_mag_i, eff_mag_v)
