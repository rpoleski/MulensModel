...
lens = mm.model.Model({
        't_0': t_0, 'u_0': u_0, 't_E': t_E, 'alpha': alpha,
        'convergence_K': K, 'shear_G': G,})
no_shear = mm.model.Model({'t_0': t_0, 'u_0': u_0, 't_E': t_E,})

lens.set_magnification_methods([min(time), 'point_source_with_shear', max(time)])
...
plt.savefig("example_21_pointlens_external_mass_sheet.png")
plt.show()